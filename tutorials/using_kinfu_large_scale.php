...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    <meta charset="utf-8" />
    <title>Using Kinfu Large Scale to generate a textured mesh &#8212; PCL 0.0 documentation</title>
...
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span>$ ./bin/pcl_kinfu_largeScale -r -et
</pre></div>
</div>
...
<blockquote>
<div><ul class="simple">
<li><p>The shifting can be triggered by rotation or translation.</p></li>
<li><p>Every time we shift out part of the cube,  four main things happen: 1)We save the data in the slice that is shifted out and send it to the world model, which is stored in CPU. 2) We clear that slice to allow for new data to be added. 3) We shift the cube’s origin. 4) We retrieve existing data (if any) from the world model and load it to the TSDF volume. This is only present when we return to areas that we previously scanned.</p></li>
<li><p>Whenever we are satisfied with the area that we have scanned, we press the “L” key to let KinFu know that we are ready to perform the exit routine. However, the routine is not executed until we shift again.</p></li>
</ul>
</div></blockquote>
...
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span>$ ./bin/pcl_kinfu_largeScale_mesh_output world.pcd
</pre></div>
</div>
...
<div class="highlight-default notranslate"><div class="highlight"><pre><span></span>$ ./bin/pcl_kinfu_largeScale_texture_output path/to/merged_mesh.ply
</pre></div>
</div>
...
<li><p><strong>Scan scenes with enough details for ICP:</strong> It is a known fact that ICP does not perform well in scenes with few details, or where there are a lot of co-planer surfaces. In other words, if the only thing you have is a wall and floor, most probably the tracking will not perform well.</p></li>
<li><p><strong>Frame rate is less than original KinFu:</strong> The code in Kinfu largescale is experimental. There are still many areas in which the performance can be optimized to provide a faster execution. In our tests, the obtained frame rate is around 20 fps. We are using a GTX480 and 4GB of RAM. The decrease in frame rate is mainly because of two things. First, that the code has not yet been completely optimized. Second, that additional operations are taking place in the frame processing loop as a result of the large scale implementation.</p></li>
<li><p><strong>Scan smoothly:</strong> Since there are more things happening per frame, KinFu Large Scale may not respond as fast as the original KinFu. Data is exchanged between GPU and CPU especially at the time of shifting. Performing smooth movements, in particular at the time of shifting, decreases the risk of losing the camera pose tracking. Be patient and you will get good results.</p></li>
</ol>
</div></blockquote>
</div>
...
<blockquote>
<div><ul class="simple">
<li><p><strong>pcl_kinfu_largeScale:</strong> In charge of obtaining the scan of the room. Its functionality is almost the same as KinFu, except that it includes the capability of shifting the cube that is being scanned to allow for large area 3D reconstruction. The output from this application is the world reconstructed model as a TSDF cloud. The concept of TSDF cloud will be explained better below. Another output from this application is a set of RGB screenshots and their corresponding camera poses.</p></li>
<li><p><strong>pcl_kinfu_largeScale_mesh_output:</strong> This application is in charge of generating a set of meshes from the extracted TSDF world cloud. The TSDF world model is processed as cubes of points and generates a mesh for each of these cubes.</p></li>
<li><p>As an additional processing step, the current state of the implementation requires that the output meshes are merged in the software of your preference. In other words, the output of the meshing application is given as a set of mesh cubes. This tutorial has been done using with Meshlab (<em>merge visible layers</em> function in Meshlab). Since the following step is performed on a per-face basis, it is also optional to decimate the mesh in order to decrease the time it takes to generate the texture.</p></li>
<li><p><strong>pcl_kinfu_largeScale_texture_output:</strong> After the meshes are generated and merged into one, this application is in charge of using the RGB screenshots and their corresponding camera poses taken during the scan to perform UV mapping in order to reconstruct the texture of the model.</p></li>
</ul>
</div></blockquote>
</div>
