...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    <meta charset="utf-8" />
    <title>How 3D Features work in PCL &#8212; PCL 0.0 documentation</title>
...
<p>From <a class="reference internal" href="#rusudissertation" id="id1"><span>[RusuDissertation]</span></a>:</p>
<blockquote>
...
<blockquote>
<div><ul class="simple">
<li><p><strong>rigid transformations</strong> - <em>that is, 3D rotations and 3D translations in the data should not influence the resultant feature vector F estimation;</em></p></li>
<li><p><strong>varying sampling density</strong> - <em>in principle, a local surface patch sampled more or less densely should have the same feature vector signature;</em></p></li>
<li><p><strong>noise</strong> - <em>the point feature representation must retain the same or very similar values in its feature vector in the presence of mild noise in the data.</em></p></li>
</ul>
...
<ul class="simple">
<li><p>determine the <strong>k</strong> (user given parameter) neighbors of a query point (also known as <em>k-search</em>);</p></li>
<li><p>determine <strong>all the neighbors</strong> of a query point within a sphere of radius <strong>r</strong> (also known as <em>radius-search</em>).</p></li>
</ul>
<div class="admonition note">
<p class="admonition-title">Note</p>
<p>For a discussion on what the right <strong>k</strong> or <strong>r</strong> values should be, please see <a class="reference internal" href="#rusudissertation" id="id2"><span>[RusuDissertation]</span></a>.</p>
</div>
</div>
...
<blockquote>
<div><ol class="arabic">
<li><p>an entire point cloud dataset, given via <strong>setInputCloud (PointCloudConstPtr &amp;)</strong> - <strong>mandatory</strong></p>
...
<li><p>a subset of a point cloud dataset, given via <strong>setInputCloud (PointCloudConstPtr &amp;)</strong> and <strong>setIndices (IndicesConstPtr &amp;)</strong> - <strong>optional</strong></p>
...
<ul>
<li><p><strong>setIndices() = false, setSearchSurface() = false</strong> - this is without a doubt the most used case in PCL, where the user is just feeding in a single PointCloud dataset and expects a certain feature estimated at <em>all the points in the cloud</em>.</p>
...
<li><p><strong>setIndices() = true, setSearchSurface() = false</strong> - as previously mentioned, the feature estimation method will only compute features for those points which have an index in the given indices vector;</p>
...
<li><p><strong>setIndices() = false, setSearchSurface() = true</strong> - as in the first case, features will be estimated for all points given as input, but, the underlying neighboring surface given in <strong>setSearchSurface()</strong> will be used to obtain nearest neighbors for the input points, rather than the input cloud itself;</p>
...
<li><p><strong>setIndices() = true, setSearchSurface() = true</strong> - this is probably the rarest case, where both indices and a search surface is given. In this case, features will be estimated for only a subset from the &lt;input, indices&gt; pair, using the search surface information given in <strong>setSearchSurface()</strong>.</p>
<p>Finally, in the figure above, this corresponds to the last (rightmost) case. Here, we assume that q_2’s index is not part of the indices vector given for Q, so no neighbors or features will be estimated at q2.</p>
</li>
</ul>
...
<span class="p">{</span>
  ...
  <span class="n">std</span><span class="o">::</span><span class="n">vector</span><span class="o">&lt;</span><span class="kt">int</span><span class="o">&gt;</span> <span class="n">indices</span> <span class="p">(</span><span class="n">std</span><span class="o">::</span><span class="n">floor</span> <span class="p">(</span><span class="n">cloud</span><span class="o">-&gt;</span><span class="n">points</span><span class="p">.</span><span class="n">size</span> <span class="p">()</span> <span class="o">/</span> <span class="mi">10</span><span class="p">));</span>
  <span class="k">for</span> <span class="p">(</span><span class="n">std</span><span class="o">::</span><span class="kt">size_t</span> <span class="n">i</span> <span class="o">=</span> <span class="mi">0</span><span class="p">;</span> <span class="n">i</span> <span class="o">&lt;</span> <span class="n">indices</span><span class="p">.</span><span class="n">size</span> <span class="p">();</span> <span class="o">++</span><span class="n">i</span><span class="p">)</span> <span class="n">indices</span><span class="p">[</span><span class="n">i</span><span class="p">]</span> <span class="o">=</span> <span class="n">i</span><span class="p">;</span>
  ...
  <span class="n">pcl</span><span class="o">::</span><span class="n">shared_ptr</span><span class="o">&lt;</span><span class="n">std</span><span class="o">::</span><span class="n">vector</span><span class="o">&lt;</span><span class="kt">int</span><span class="o">&gt;</span> <span class="o">&gt;</span> <span class="n">indicesptr</span> <span class="p">(</span><span class="k">new</span> <span class="n">std</span><span class="o">::</span><span class="n">vector</span><span class="o">&lt;</span><span class="kt">int</span><span class="o">&gt;</span> <span class="p">(</span><span class="n">indices</span><span class="p">));</span>
...
<dl class="citation">
<dt class="label" id="rusudissertation"><span class="brackets">RusuDissertation</span><span class="fn-backref">(<a href="#id1">1</a>,<a href="#id2">2</a>,<a href="#id3">3</a>)</span></dt>
<dd><p><a class="reference external" href="http://mediatum.ub.tum.de/doc/800632/941254.pdf">http://mediatum.ub.tum.de/doc/800632/941254.pdf</a></p>
</dd>
</dl>
<div class="admonition note">
<p class="admonition-title">Note</p>
<p>&#64;PhDThesis{RusuDoctoralDissertation,
